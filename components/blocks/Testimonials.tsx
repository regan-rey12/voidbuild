...
export default function Testimonials({ data, style, editMode, onUpdateData }: TestimonialsProps) {
  ...
  const badgeLabel = variant === 'restaurant' ? 'Guest Favorite' : isBoutique ? 'Style Favorite' : isPharmacy ? 'Verified Customer' : isHotel ? 'Guest Review' : isBarbershop ? 'Regular Client' : isHardware ? 'Verified Buyer' : 'Verified Client';
          ...
          {testimonials.map((t, i) => (
            <div key={i} className={`p-6 rounded-2xl border shadow-sm flex flex-col justify-between hover:shadow-md transition ${variant === 'salon' ? 'bg-white border-rose-100' : isBoutique ? 'bg-white border-stone-200' : isPharmacy ? 'bg-white border-emerald-100' : isHotel ? 'bg-white border-stone-200' : isBarbershop || isHardware ? 'bg-white border-stone-200' : 'bg-white border-gray-200'}`}>
              <div>
                ...
                <div className={`mt-4 text-gray-700 leading-relaxed italic ${isBoutique || isPharmacy || isHotel || isBarbershop || isHardware ? 'text-sm md:text-[15px]' : 'text-xs md:text-sm'}`}>
                  &ldquo;
                  {editMode && onUpdateData ? (
                    <EditableText value={t.text} onChange={(v) => updateTestimonial(i, 'text', v)} editMode={editMode} as="span" multiline className={isBoutique || isPharmacy || isHotel || isBarbershop || isHardware ? 'text-sm md:text-[15px]' : 'text-xs md:text-sm'} />
                  ) : (
                    t.text
                  )}
                  &rdquo;
              ...
              <div className="mt-6 pt-4 border-t border-gray-100 flex items-center justify-between gap-3">
                ...
                <div className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${variant === 'salon' ? 'bg-rose-50 text-rose-700 border-rose-200' : variant === 'restaurant' ? 'bg-amber-50 text-amber-700 border-amber-200' : isBoutique ? 'bg-stone-100 text-stone-700 border-stone-200' : isPharmacy ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : isHotel ? 'bg-emerald-50 text-emerald-800 border-emerald-200' : isBarbershop ? 'bg-amber-50 text-amber-800 border-amber-200' : isHardware ? 'bg-orange-50 text-orange-800 border-orange-200' : 'bg-green-50 text-green-700 border-green-200'}`} style={variant === 'salon' ? { color: primary } : undefined}>
                  {badgeLabel}
                </div>
              </div>
            </div>
