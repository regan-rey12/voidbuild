...
export default function Services({ data, style, editMode, onUpdateData }: ServicesProps) {
  ...
  const addService = () => {
    if (!onUpdateData) return;
    onUpdateData({ ...data, services: [...data.services, { name: 'New Service', price: 'UGX 20,000', description: 'Describe service', image: 'business' }] });
  };
      ...
      <div className="max-w-6xl mx-auto">
        <h2 className="text-xl md:text-3xl font-bold text-center">
          {editMode && onUpdateData ? (
            <EditableText value={data.heading || 'Our Services'} onChange={(v) => updateField('heading', v)} editMode={editMode} as="span" className="text-xl md:text-3xl font-bold" />
          ) : (
          ...
          {data.services?.map((service, i) => (
            <div key={i} className="group bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg hover:border-gray-300 transition-all flex flex-col">
              <div className="relative aspect-[16/9] md:aspect-[4/3] bg-gray-100">
                {editMode && onUpdateData ? (
                  <EditableImage imageKeyword={service.image || 'business'} alt={service.name} editMode={editMode} onChange={(url) => updateService(i, 'image', url)} className="w-full h-full object-cover" />
                ) : (
        ...
        {editMode && onUpdateData && (
          <div className="mt-6 text-center">
            <button onClick={addService} className="px-4 py-2 rounded-full bg-white border text-xs font-semibold">+ Add Service</button>
          </div>
        )}
      </div>
    </section>
