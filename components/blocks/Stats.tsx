        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
          {stats.map((s, i) => (
            <div key={i} className={isBoutique ? 'p-6 rounded-[24px] bg-white border border-stone-200 shadow-sm' : isPharmacy ? 'p-6 rounded-[24px] bg-white border border-emerald-100 shadow-sm' : isClinic ? 'p-6 rounded-[24px] bg-white border border-cyan-100 shadow-sm' : isSalon ? 'p-6 rounded-[24px] bg-white border border-rose-100 shadow-sm' : isBakery ? 'p-6 rounded-[24px] bg-white border border-amber-100 shadow-sm' : isHotel || isBarbershop || isHardware || isGym || isLaundry || isIt || isTutoring || isSchool || isPortfolio || isRestaurant ? 'p-6 rounded-[24px] bg-white/[0.06] border border-white/10' : 'p-4 rounded-2xl bg-gray-50 border border-gray-100'}>
              ...
              <div className={`mt-1.5 font-medium ${isBoutique || isPharmacy || isClinic || isBakery || isSalon ? 'text-[13px] md:text-sm text-gray-600' : isHotel || isBarbershop || isHardware || isGym || isLaundry || isIt || isTutoring || isSchool || isPortfolio || isRestaurant ? 'text-[13px] md:text-sm text-white/75' : 'text-xs md:text-sm text-gray-600'}`}>
                {editMode && onUpdateData ? (
                  <EditableText value={s.label} onChange={(v) => updateStat(i, 'label', v)} editMode={editMode} as="span" className={isBoutique || isPharmacy || isHotel || isBarbershop || isHardware || isClinic || isBakery || isGym || isLaundry || isIt || isTutoring || isSchool || isPortfolio || isRestaurant || isSalon ? 'text-[13px] md:text-sm font-medium' : 'text-xs md:text-sm font-medium'} />
                ) : (
                  s.label
                )}
              </div>
