...
export default function Pricing({ data, style, projectId, editMode, onUpdateData }: PricingProps) {
  ...
  const pillText = isBoutique ? 'Private Fittings • Kampala Delivery • Clear UGX Pricing' : isPharmacy ? 'Licensed care • Refill support • Delivery available' : isHotel ? 'Breakfast included • Couples & family packages • Flexible dates' : isBarbershop ? 'Walk-ins welcome • Clear UGX prices • Open till late' : isHardware ? 'Direct factory prices • Site delivery • Retail & wholesale' : 'Fair Prices • Transparent UGX • MTN MoMo Accepted';
  const buttonLabel = data.ctaText || (variant === 'salon' ? 'Book Package' : variant === 'restaurant' ? 'Choose Combo' : isBoutique ? 'Ask About This Option' : isPharmacy ? 'Ask About This Service' : isHotel ? 'Enquire About This Package' : isBarbershop ? 'Book This Package' : isHardware ? 'Request This Package' : 'Choose Package');
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center max-w-2xl mx-auto">
          <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-xs font-semibold mb-3 shadow-sm ${variant === 'salon' ? 'bg-white text-rose-900 border-rose-100' : isBoutique ? 'bg-white text-stone-800 border-stone-200' : isPharmacy ? 'bg-white text-emerald-800 border-emerald-100' : isHotel ? 'bg-white text-emerald-900 border-emerald-100' : isBarbershop ? 'bg-white text-stone-900 border-stone-300' : isHardware ? 'bg-white text-slate-900 border-slate-300' : 'bg-white text-gray-800'}`}>
            <Sparkles className={`w-3.5 h-3.5 ${variant === 'salon' ? 'text-rose-500' : isBoutique ? 'text-amber-700' : isPharmacy ? 'text-emerald-600' : isHotel ? 'text-emerald-700' : isBarbershop ? 'text-amber-600' : isHardware ? 'text-orange-600' : 'text-yellow-500'}`} />
            <span>{pillText}</span>
          </div>
          ...
          {plans.map((p, i) => {
            ...
            return (
                ...
                className={`relative rounded-2xl p-6 border bg-white flex flex-col justify-between transition ${
                  p.popular
                    ? variant === 'salon'
                      ...
                      : isBarbershop
                      ? 'bg-[#17140f] border-amber-500/40 shadow-xl ring-2 ring-amber-400/15'
                      : isHardware
                      ? 'bg-[#16283a] border-orange-500/40 shadow-xl ring-2 ring-orange-400/15'
              ...
              >
                <div>
                  {p.popular && (
                    <div className={`text-[9px] font-extrabold uppercase tracking-wider inline-flex px-2.5 py-0.5 rounded-full mb-2 ${variant === 'salon' ? 'bg-rose-500 text-white' : variant === 'restaurant' ? 'bg-amber-500 text-white' : variant === 'boutique' ? 'bg-stone-900 text-[#f7eadb]' : variant === 'pharmacy' ? 'bg-emerald-600 text-white' : isHotel ? 'bg-emerald-800 text-white' : isBarbershop ? 'bg-[#C9962E] text-[#221604]' : isHardware ? 'bg-[#C2410C] text-white' : 'bg-yellow-400 text-gray-950'}`}>
                      MOST POPULAR
                    </div>
                  )}

                  <div className={`font-bold text-sm ${(isBarbershop || isHardware) && p.popular ? 'text-white' : 'text-gray-900'}`}>
                    {editMode && onUpdateData ? (
                  ...
                  <div className={`text-2xl md:text-3xl font-extrabold mt-2 ${(isBarbershop || isHardware) && p.popular ? 'text-white' : 'text-gray-900'}`}>
                    {editMode && onUpdateData ? (
                  ...
                  {p.sub && <div className={`text-[11px] mt-0.5 ${(isBarbershop || isHardware) && p.popular ? 'text-white/55' : 'text-gray-500'}`}>{p.sub}</div>}

                  <ul className={`mt-5 space-y-2.5 text-xs ${(isBarbershop || isHardware) && p.popular ? 'text-white/75' : 'text-gray-600'}`}>
                    {p.features?.map((f, j) => (
                      <li key={j} className="flex items-start gap-2">
                        <Check className={`w-3.5 h-3.5 flex-shrink-0 mt-0.5 ${isBoutique ? 'text-stone-700' : isPharmacy ? 'text-emerald-600' : isHotel ? 'text-emerald-700' : isBarbershop ? (p.popular ? 'text-amber-400' : 'text-amber-700') : isHardware ? (p.popular ? 'text-orange-400' : 'text-orange-700') : 'text-green-600'}`} />
                        <span>{f}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <div className={`mt-6 pt-4 border-t ${(isBarbershop || isHardware) && p.popular ? 'border-white/10' : 'border-gray-100'}`}>
                  <a
                    href={planWaLink}
                    target={waNumber ? '_blank' : '_self'}
                    rel={waNumber ? 'noopener noreferrer' : undefined}
                    onClick={(e) => handlePlanClick(e)}
                    className={`w-full py-2.5 rounded-xl text-xs font-bold text-center flex items-center justify-center gap-1.5 shadow-sm hover:opacity-95 transition ${isBarbershop && p.popular ? 'text-[#221604]' : 'text-white'}`}
                    style={{ backgroundColor: isBarbershop && p.popular ? '#C9962E' : isHardware && p.popular ? '#C2410C' : primary }}
                  >
