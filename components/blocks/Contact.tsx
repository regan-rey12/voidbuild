...
export default function Contact({ data, style, projectId, editMode, onUpdateData }: ContactProps) {
  ...
  return (
    <section id="contact" className={isBoutique ? 'py-16 md:py-20 px-4 md:px-6 bg-[#fdfaf6]' : isPharmacy ? 'py-16 md:py-20 px-4 md:px-6 bg-[#f7fcfa]' : 'py-16 px-6 bg-white'}>
      <div className="max-w-6xl mx-auto grid md:grid-cols-5 gap-8 items-start">
        <div className="md:col-span-2">
          <div className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold mb-3 ${isBoutique ? 'bg-stone-100 text-stone-800 border border-stone-200' : isPharmacy ? 'bg-emerald-50 text-emerald-800 border border-emerald-100' : 'bg-gray-100 text-gray-800'}`}>
            {isPharmacy ? 'Talk to the Pharmacy' : 'Get in Touch'}
          </div>
          <h2 className={isBoutique || isPharmacy ? 'text-2xl md:text-4xl font-extrabold tracking-tight text-gray-900' : 'text-2xl font-bold tracking-tight'}>
          ...
          <p className={isBoutique || isPharmacy ? 'mt-3 text-sm md:text-base text-gray-600 leading-relaxed' : 'mt-2 text-sm text-gray-600'}>
          ...
          <div className="mt-6 space-y-4 text-sm">
            {data.location && (
              <div className="flex gap-3">
                <div className={`w-9 h-9 rounded-lg border flex items-center justify-center flex-shrink-0 ${isBoutique ? 'bg-white border-stone-200' : isPharmacy ? 'bg-white border-emerald-100' : 'bg-gray-50 border-gray-200'}`}><MapPin className="w-4 h-4 text-gray-600" /></div>
                <div>
            ...
            {data.phone && (
              <div className="flex gap-3">
                <div className={`w-9 h-9 rounded-lg border flex items-center justify-center flex-shrink-0 ${isBoutique ? 'bg-white border-stone-200' : isPharmacy ? 'bg-white border-emerald-100' : 'bg-gray-50 border-gray-200'}`}><Phone className="w-4 h-4 text-gray-600" /></div>
                <div>
            ...
            {data.hours && (
              <div className="flex gap-3">
                <div className={`w-9 h-9 rounded-lg border flex items-center justify-center flex-shrink-0 ${isBoutique ? 'bg-white border-stone-200' : isPharmacy ? 'bg-white border-emerald-100' : 'bg-gray-50 border-gray-200'}`}><Clock className="w-4 h-4 text-gray-600" /></div>
                <div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-2">
            ...
            >
              ...
              <span>{isPharmacy ? 'Message Pharmacy' : 'Message Store'}</span>
            </a>
            {data.phone && (
        ...
        <div className={`md:col-span-3 rounded-xl p-1 border ${isBoutique ? 'bg-[#f4ede6] border-stone-200' : isPharmacy ? 'bg-[#ecfdf5] border-emerald-100' : 'bg-gray-50 border-gray-200'}`}>
          <div className={`rounded-lg p-5 border shadow-sm ${isBoutique ? 'bg-white border-stone-200' : isPharmacy ? 'bg-white border-emerald-100' : 'bg-white border-gray-200'}`}>
            ...
            <p className="text-xs text-gray-500 mt-1">Your message opens directly in WhatsApp for fast follow-up.</p>
