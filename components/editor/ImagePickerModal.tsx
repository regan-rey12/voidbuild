...
export default function ImagePickerModal({ isOpen, onClose, onSelect, currentKeyword, category }: Props) {
  ...
  // FIXED: No body overflow hidden that causes navbar shift right
  // Instead, just prevent background scroll without hiding scrollbar
  useEffect(() => {
    if (isOpen) {
      document.body.style.overflow = 'hidden';
      // Keep scrollbar gutter stable, no shift
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col animate-in fade-in zoom-in-95">
        ...
        <div className="flex gap-2 p-3 border-b bg-gray-50 flex-shrink-0">
          ...
          <button onClick={() => setTab('upload')} className={`px-4 py-2 rounded-full text-sm font-medium transition ${tab==='upload' ? 'bg-gray-900 text-white' : 'bg-white border hover:bg-gray-50'}`}>Upload</button>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {tab === 'gallery' && (
            <div>
              <div className="text-xs text-gray-500 mb-3">Curated for Uganda — click to select</div>
          ...
          {tab === 'upload' && (
            <div className="text-center py-8">
              ...
              <div className="text-sm text-gray-500 mt-1 max-w-sm mx-auto">Take photo with phone — saves inside your site</div>
        ...
        <div className="p-4 border-t bg-gray-50 rounded-b-2xl flex justify-between items-center flex-shrink-0">
          <div className="text-[10px] text-gray-500">Real photo = +50% trust vs emoji</div>
