        ...
        <div className="max-w-6xl md:max-w-[1600px] mx-auto px-3 md:px-6 h-12 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <button
              ...
              className="md:hidden w-8 h-8 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-700"
            ...
            {currentPage !== 'home' && (
              <span className="hidden md:inline-flex ml-2 text-[11px] px-2 py-0.5 rounded-full bg-gray-100 border capitalize text-gray-700 font-medium">{currentPage}</span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            <div className="hidden md:flex items-center gap-1 mr-1">
              {navLinks.map(link => (
                <Link
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium hover:bg-gray-50 transition ${currentPage === link.page ? 'bg-gray-100 text-black' : 'text-gray-600'}`}
                >
                  {link.label}
                </Link>
              ))}
            ...
            {showBuilderActions && builderActions ? (
              ...
              <Link href="/builder" className="hidden md:inline-flex px-4 py-1.5 rounded-full bg-gray-900 text-white text-xs font-bold hover:bg-black transition shadow-sm">
                Build Website
              </Link>
            )}
          ...
          <div className="absolute left-0 top-0 h-full w-[82%] max-w-[300px] bg-white shadow-2xl flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4">
              <div className="space-y-1">
                <Link
                  href="/"
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm ${currentPage === 'home' ? 'bg-gray-100 font-medium' : 'hover:bg-gray-50'}`}
                  onClick={(e) => {
                    setMobileMenuOpen(false);
                    handleNavClick(e, '/#templates');
                  }}
                >
                  Home
                </Link>
